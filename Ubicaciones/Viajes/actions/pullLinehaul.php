...
$query = "SELECT lh.pk_idlinehaul linehaulid, lh.pk_linehaul_number lh_number, lh.fk_tripyear trip_year, lh.linehaul_status lh_status, lh.fk_idtrip tripid , lh.date_begin trip_start , lh.date_end trip_end , lh.trip_rate rate , lh.fuel_surcharge fuel_surcharge , lh.origin_state origin_state , lh.origin_city origin_city , lh.origin_zip origin_zip , lh.destination_state destination_state , lh.destination_city destination_city , lh.destination_zip destination_zip , lh.fkid_broker brokerid , b.brokerName broker , sum(tlm.miles_google) total_miles , SUM( CASE tlm.movement_type WHEN 'E' THEN tlm.miles_google ELSE 0 END) empty_miles , SUM( CASE tlm.movement_type WHEN 'L' THEN tlm.miles_google ELSE 0 END) loaded_miles, date_departure departure, date_arrival arrival, date_delivery delivery, date_appointment appointment, broker_reference broker_reference, lh.linehaul_comment lh_comment FROM ct_trip_linehaul lh LEFT JOIN ct_brokers b ON lh.fkid_broker = b.pkid_broker LEFT JOIN ct_trip_linehaul_movement tlm ON tlm.fkid_linehaul = lh.pk_idlinehaul WHERE pk_idlinehaul = ? GROUP BY linehaulid";
