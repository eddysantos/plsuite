...
$query = "SELECT tl.invoice_payment_date lh_date , $period(tl.invoice_payment_date) date_grouping, sum(tl.invoice_amount) rate FROM ct_trip t LEFT JOIN ct_trip_linehaul tl ON t.pkid_trip = tl.fk_idtrip WHERE tl.invoice_payment_date BETWEEN ? AND ? AND tl.linehaul_status <> 'Cancelled' $and_where GROUP BY date_grouping $group_by";
...
$query = "SELECT tl.invoice_payment_due due_date , $period(tl.invoice_payment_due) date_grouping, sum(tl.invoice_amount) rate FROM ct_trip t LEFT JOIN ct_trip_linehaul tl ON t.pkid_trip = tl.fk_idtrip WHERE tl.invoice_payment_due BETWEEN ? AND ? AND tl.linehaul_status <> 'Cancelled' $and_where GROUP BY date_grouping $group_by";
...
while ($row = $rslt->fetch_assoc()) {
  // $results[$row['date_grouping']]['date'] = $row['lh_date'];
  $results[$row['date_grouping']]['amount_due'] += $row['rate'];
}
...
$chart_data = array(['x'],['Collected'],['Goal']);
...
switch ($data['period']) {
  case 0:
    foreach ($results as $date_group => $result) {
      array_push($chart_data[0], $date_group);
      array_push($chart_data[1], $result['rate']);
      array_push($chart_data[2], $result['amount_due']);
    }
    break;
