...
$query = "INSERT INTO omni_pos_log(tran_id, tran_ts, driverid1, driverid2, tractor, lat, lon, speed, heading) VALUES (?,?,?,?,?,?,?,?,?) ON DUPLICATE KEY UPDATE tran_id = ?, tran_ts = ?, driverid1 = ?, driverid2 = ?, tractor = ?, lat = ?, lon = ?, speed = ?, heading = ?";
...
do {
  ...
  foreach ($transactions as $transaction) {
    ...
    $insert_pos_log->bind_param('ssssssssssssssssss', $position_id, $event_ts, $driver, $driver2, $tractor, $lat, $lon, $speed, $heading, $position_id, $event_ts, $driver, $driver2, $tractor, $lat, $lon, $speed, $heading) or die('Error binding: ' . $insert_pos_log->error);
